...
class Database:
    ...
    def _conn(self):
        """Borrow a connection from the pool; always return it even on exception."""
    ...
    def init_database(self):
        with self._conn() as conn:
            ...
            # --- Indexes ---
            # conversations: ดึงรายการแชทของ user เรียงตาม updated_at
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_conversations_user_active_updated ON conversations (user_id, is_active, updated_at DESC)")
            # messages: ดึงข้อความในแชท เรียงตาม created_at
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_messages_conversation_created ON messages (conversation_id, created_at ASC)")
            # messages: นับเฉพาะ sender='assistant' ใน feedback summary
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages (sender)")
            # activity_logs: ดึง log ของ user เรียงตาม created_at
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_activity_logs_user_created ON activity_logs (user_id, created_at DESC)")
            # intent_logs: ดึง log ตาม conversation
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_intent_logs_conversation ON intent_logs (conversation_id, created_at DESC)")
            # mcp_interactions: ดึง interaction ตาม conversation
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_mcp_interactions_conversation ON mcp_interactions (conversation_id, created_at DESC)")
            # suggestions: ดึง suggestion ตาม conversation
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_suggestions_conversation ON suggestions (conversation_id)")
            # token_usage: JOIN กับ users ใน admin dashboard
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_token_usage_user ON token_usage (user_id)")
            # support_tickets: ดึง ticket ของ user เรียงตาม updated_at
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_support_tickets_user_updated ON support_tickets (user_id, updated_at DESC)")
            # support_replies: ดึง reply ตาม ticket เรียงตาม created_at
    ...
    def get_conversation_messages(self, conversation_id, user_id, limit=200):
        with self._conn() as conn:
            ...
            cursor.execute('''
                ...
                LEFT JOIN message_feedback mf ON mf.message_id = m.id AND mf.user_id = %s
                WHERE m.conversation_id = %s AND c.user_id = %s AND c.is_active = TRUE
                ORDER BY m.created_at ASC
                LIMIT %s
            ''', (user_id, conversation_id, user_id, limit))
            messages = []
    ...
    def get_ticket_replies(self, ticket_id, user_id=None):
        with self._conn() as conn:
            ...
            if user_id is not None:
                cursor.execute(
                    "SELECT id FROM support_tickets WHERE id = %s AND user_id = %s",
                    (ticket_id, user_id)
                )
